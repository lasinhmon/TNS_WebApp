...
namespace TNS_KPI.Areas.KPI_Accounting.Pages
{
    ...
    public class AgritaxModel : PageModel
    {
        #region [ Security ]
        ...
        public IActionResult OnPostLoadData([FromBody] ItemRequest request)
        {
            ...
            string zListField = "SoChungTu,NgayChungTu,MaNhanVien";
           
            string zWhere = " WHERE InMonth = " + request.MonthView + " AND InYear = " + request.YearView;
            if (request.SearchContent.Trim().Length > 0)
                zWhere += " AND MaNhanVien LIKE '%" + request.SearchContent +"%'";
            string zSQL = "SELECT " + zListField + " FROM [dbo].[KPI_Accounting_Agritax] " + zWhere + " ORDER BY NgayChungTu ";
            string zSQL_Count = "SELECT Count(*) FROM [dbo].[KPI_Accounting_Agritax] " + zWhere;
        ...
        {
            CheckAuth();
            List<string[]> zData = Models.AnalyticsData.ReportQuick_Argitax(int.Parse(request.MonthView), int.Parse(request.YearView));
