...
namespace TNS_KPI.Areas.KPI_Accounting.Pages
{
    ...
    public class KetQuaBTModel : PageModel
    {
        #region [ Security ]
        ...
        //public List<string[]> ListDepartment = Models.AccessData.Accouting_Result();   
        ...
        public IActionResult OnPostView([FromBody] ItemRequest request)
        {
            ...
            List<ViewItem> zList = new List<ViewItem>();
            ViewItem zItem = new ViewItem();
            ...
            else
            {
                ...
                foreach (string[] zInfo in zListEmployee)
                {
                    ...
                    zItem.Ranking = zInfo[0];
                    zItem.UserName = zInfo[1];
                    zItem.EmployeeName = zInfo[2];
                    zItem.ButToanThucHien = zInfo[3];
                    zItem.ButToanHuy = zInfo[4];
                    zItem.ButToanAgritax = zInfo[5];
                    zItem.ButToanKhongHachToan = zInfo[6];
                    zItem.ButToanPHUB = zInfo[7];
                    zItem.HauKiemButToan = zInfo[8];
                    zItem.TongDiemQuyDoi = zInfo[9];
                    zItem.DiemCongPhuTrach = zInfo[10];
                    zItem.HeSoCongViec = zInfo[11];
                    zItem.GDV_KS_ToTruong_PhuTrach = zInfo[12];
                    zItem.DiemThucHien = zInfo[13];
                    zItem.DiemBTBQ = zInfo[14];
                    zItem.DiemBTBQ_Rate = zInfo[15];
                    zItem.Rate_Target   = zInfo[16];
                    zItem.Note = zInfo[17];
                    zList.Add(zItem);
                }
            }
        ...
        {
            //Ranking, UserName, EmployeeName, ButToanThucHien, ButToanHuy, ButToanAgritax, ButToanKhongHachToan, 
