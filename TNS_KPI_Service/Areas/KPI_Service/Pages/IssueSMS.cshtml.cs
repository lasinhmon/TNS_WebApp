...
namespace TNS_KPI.Areas.KPI_Service.Pages
{
    ...
    public class IssueSMSModel : PageModel
    {
        #region [ Security ]
        ...
        public List<string[]>? zListPGD = Models.Categories_AccessData.SMS_GetPGD(DateTime.Now.Month, DateTime.Now.Year);
        public List<string[]>? zListSTTS = Models.Categories_AccessData.SMS_GetSTTS(DateTime.Now.Month, DateTime.Now.Year);
        ...
        {
            string zMessageLog = "";
            Models.SMS_Model sMS_Model = new Models.SMS_Model();
            sMS_Model.Crtusr = itemUpdate.crtusr;
            sMS_Model.Pgd = itemUpdate.pgd;
            zMessageLog = sMS_Model.Save();
            // sMS_Model.EmployeeID = itemUpdate.ID;
            //  sMS_Model.FirstName = itemUpdate.FirstName;
            //  sMS_Model.LastName = itemUpdate.LastName;
        ...
        public IActionResult OnPostCreateUser([FromBody] ItemRequest request)
        {
            CheckAuth();
            string zMessageLog = "";
            Models.SMS_Model sMS_Model;
            sMS_Model = new Models.SMS_Model();
            ItemView zItem = new ItemView();
            return new JsonResult(zItem);
        }
        // Bi troi Index, tam thoi sua vi tri cot
        public IActionResult OnPostDelete1([FromBody] ItemView itemUpdate)
        {
            string zMessageLog = "";
            zMessageLog = Models.Categories_AccessData.Delete1(itemUpdate.crtusr);
            itemUpdate.Code = zMessageLog;
            return new JsonResult(itemUpdate);
        }
        public IActionResult OnPostUpdate1([FromBody] ItemView itemUpdate)
        {
            //CheckAuth();
            string zMessageLog = "";
            zMessageLog = Models.Categories_AccessData.Update1(itemUpdate.crtusr, itemUpdate.pgd);
        ...
        public IActionResult OnPostLoadData([FromBody] ItemRequest request)
        {
            ...
            string zListField = "crtusr,entydt,stts,custseq,mblno1,rmk,crtdtm, nmloc, custtpcd";
            ...
            string zSQL = "SELECT " + zListField + " FROM [dbo].[KPI_Service_IssueSMS] " + zWhere + " ORDER BY entydt DESC";
            string zSQL_Count = "SELECT Count(*) FROM [dbo].[KPI_Service_IssueSMS] " + zWhere;
        ...
        {
            CheckAuth();
            List<string[]> zData = Models.AnalyticsData.ReportQuick_SMS(int.Parse(request.MonthView), int.Parse(request.YearView), request.StatusView);
        ...
        public IActionResult OnPostSearchPGD([FromBody] ItemRequest request)
        {
            ...
            zListPGD = Models.Categories_AccessData.SMS_GetPGD(zMonth, zYear);
